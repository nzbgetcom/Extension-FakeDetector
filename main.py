...
def start_check():
    ...
    if (
        not ("NZBNA_EVENT" in os.environ or "NZBPP_DIRECTORY" in os.environ)
        or not "NZBOP_ARTICLECACHE" in os.environ
    ):
    ...
    if os.environ.get("NZBNA_EVENT") not in [
        "NZB_ADDED",
        "FILE_DOWNLOADED",
        "NZB_DOWNLOADED",
        None,
    ...
    if "NZBPP_DIRECTORY" in os.environ and not os.path.exists(
        os.environ.get("NZBPP_DIRECTORY")
    ):
...
def detect_fake(name, dir):
    ...
    filelist.extend(
        [o for o in os.listdir(dir) if os.path.isfile(os.path.join(dir, o))]
    )
    dirlist = [
        os.path.join(dir, o)
...
def main():
    ...
    tmp_file_name = (
        os.environ.get("NZBOP_TEMPDIR")
        + "/FakeDetector/"
        + os.environ.get(Prefix + "NZBID")
    )
    ...
    if os.environ.get("NZBNA_EVENT") == "NZB_ADDED" or (
        os.environ.get("NZBNA_EVENT") == "FILE_DOWNLOADED"
        and os.environ.get("NZBPR_FAKEDETECTOR_SORTED") != "yes"
    ):
